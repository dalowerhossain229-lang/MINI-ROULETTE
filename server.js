...
// 🎡 মিনি রুলেটের ওরিজিনাল ১২টি সংখ্যার কালার ম্যাপিং অবজেক্ট কন্টেইনার লক (আপনার স্ক্রিনশটের চাকা সিকোয়েন্স সিঙ্ক!)
...
app.post('/api/roulette-deal', async (req, res) => {
    const { userId, amount, wallet, prediction } = req.body; // prediction: 'RED', 'BLACK', 'ZERO'
    const reqAmount = parseFloat(amount) || 50;
    const userPrediction = String(prediction || "RED").toUpperCase(); 
    const finalGameName = "miniroulette"; // 🎯 লবির কি-শর্টকোড টাইট লক
    const targetWallet = wallet || "main";
    ...
    try {
        ...
        while (isLoopActive && loopSafety < 150) {
            ...
            winningSlotIndex = Math.floor(Math.random() * rouletteWheelNumbers.length);
            selectedSlotObject = rouletteWheelNumbers[winningSlotIndex];
            ...
            if (balResponse.data && balResponse.data.roulette_target) {
                ...
                if (finalStatus === "win") {
                    // মিনি রুলেট সুপ্রিম আরটিপি স্বাভাবিক ক্যাসিনো ট্র্যাকে ৪৩% এ ব্যালেন্সড লক ভাই ভাই!
